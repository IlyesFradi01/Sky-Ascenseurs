  ...
  return (
    <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
      ...
      <ul className="space-y-4">
        {files.map((f) => (
          <li key={f.href} className="flex items-center justify-between p-5 rounded-2xl border border-gray-200 bg-white shadow-sm">
            <div>
              <p className="font-semibold text-gray-900">{f.name}</p>
              <p className="text-gray-600 text-sm">{f.size}</p>
            </div>
            <a href={f.href} className="px-5 py-2 rounded-full bg-[#428bca] hover:bg-[#357abd] text-white transition">
              Download
            </a>
          </li>
        ))}
